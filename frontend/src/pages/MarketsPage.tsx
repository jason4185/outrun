...
import { ASSETS, CATEGORY_LABEL, CATEGORIES, type Category, type MarketStatus } from "../lib/outrun/types";
...
export function MarketsPage() {
  const { markets, marketsLoading, marketsError, refetchMarkets } = useOutrun();
  ...
  const filtered = useMemo(() => markets.filter((market) => {
    ...
    const searchMatch = !needle || [marketQuestionText(market.category), CATEGORY_LABEL[market.category], ...ASSETS[market.category]].join(" ").toLowerCase().includes(needle);
  ...
  return <PageContainer>
    ...
    <div className="filters"><div className="tab-list" role="tablist" aria-label="Market categories"><button className={`tab${category === "ALL" ? " active" : ""}`} type="button" onClick={() => setCategory("ALL")}>All</button>{CATEGORIES.map((item) => <button className={`tab${category === item ? " active" : ""}`} type="button" key={item} onClick={() => setCategory(item)}>{CATEGORY_LABEL[item]}</button>)}</div><select aria-label="Filter market status" className="filter-select" value={status} onChange={(event) => setStatus(event.target.value)}>{STATUS_FILTERS.map((item) => <option value={item.value} key={item.value}>{item.label}</option>)}</select></div>
