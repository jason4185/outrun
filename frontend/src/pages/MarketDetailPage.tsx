...
export function MarketDetailPage() {
  ...
  return <PageContainer>
    ...
    <div className="detail-layout"><div className="left-stack">
      <section className="surface detail-card"><div className="card-heading"><div><h2>{section === "performance" ? chartTitle : "Pool Composition"}</h2><p>{section === "performance" ? chartSubtitle : "How GEN is distributed across the three competitors"}</p></div><div className="segmented"><button className={`segment${section === "performance" ? " active" : ""}`} type="button" onClick={() => setSection("performance")}>{chartTitle}</button><button className={`segment${section === "pool" ? " active" : ""}`} type="button" onClick={() => setSection("pool")}>Pool Composition</button></div></div>{section === "performance" ? chartQuery.isError ? <div className="chart-unavailable"><strong>Live chart temporarily unavailable</strong><span>We couldn't load Binance market data. We'll retry shortly.</span></div> : <><PerformanceChart market={chartMarket} /><InfoStrip><strong>{phase === "PRE_MARKET" ? "Pre-market data is informational only." : phase === "ACTIVE" ? "Live performance is informational only." : "Market performance is informational only."}</strong> {phase === "PRE_MARKET" ? `Market performance resets at ${formatWindow(market.start, market.start).split(" →")[0]} UTC.` : "Settlement uses Bybit, Gate and Bitget with 2-of-3 source consensus."}</InfoStrip></> : <PoolComposition market={market} />}</section>
